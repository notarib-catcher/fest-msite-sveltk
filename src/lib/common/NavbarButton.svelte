...
{#if isLogoutbtn}
<button type="button" class="text-white border-x-zinc-600 font-extralight from-[#D283B8] via-[#995BBB] to-[#6738BE] bg-clip-text hover:text-transparent hover:font-semibold bg-gradient-to-r active:font-semibold max-md:text-left active:text-transparent text-xl  duration-200" on:click={($page.data.session)?async ()=>{ await signOut({callbackUrl: "/?logout"})}:() => {signIn("google", {callbackUrl: window.location.href + (window.location.href.endsWith('/')?"" : "/") + "?loginSuccess"})}} data-sveltekit-preload-data><div class="px-16 max-lg:px-5 max-xl:px-10 py-2.5">{($page.data.session)?"Logout":"Login"}</div></button>
{:else}
