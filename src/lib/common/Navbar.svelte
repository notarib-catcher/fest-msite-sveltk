...
<div id="nav-container" class=" w-screen {$page.url.pathname == "/"?"":"fixed"}  top-0 z-10">
    ...
    <div id="coverall" class=" overscroll-contain fadeIn max-md:hide max-md:h-screen w-screen max-md:fixed z-[8] max-md:backdrop-blur-md max-md:top-0 max-md:left-0 md:pointer-events-none max-md:fadeIn max-md:hidden overflow-hidden">
        <nav id="nav" class=" md:shadow-lg bg-opacity-80 pointer-events-auto   max-md:hidden md:w-full md:h-full bg-zinc-900 flex md:flex-row-reverse flex-col left-0 h-screen max-md:fixed sticky max-md:min-w-[40%] max-md:w-fit  max-w-full">
                ...
                <div class=" my-auto h-5 whitespace-nowrap flex-nowrap">
                    <div class=" grid md:grid-flow-col md:grid-rows-1 grid-flow-row grid-cols-1">
                        ...
                        <div class=" grid grid-flow-col grid-rows-1">
                            <div id="tappitytap" class=" max-md:hidden">Hello</div><div id="cursor" class=" loopOpacity font-bold mr-1 max-md:hidden  bg-clip-text text-transparent bg-gradient-to-b from-[#D283B8] to-[#D283B8] via-[#6738BE]">|</div>
                        </div>
                        <div class=" grid grid-flow-col grid-rows-1 bg-gradient-to-tr font-semibold via-[#59e8f8] to-[#6088f9] from-[#6088f9] bg-clip-text whitespace-nowrap text-transparent backgroundpan"> {#if $page.data.session}{$page.data.session.user?.name?.split(" ")[0]}{:else}<div class="max-md:hidden bg-gradient-to-tr font-semibold from-[#D283B8] via-[#995BBB] to-[#6738BE] bg-clip-text whitespace-nowrap text-transparent">explorer</div>{/if}<div class=" max-md:hidden">!</div></div>
                    </div>
