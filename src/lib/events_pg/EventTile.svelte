...
<script>
	...
	<div class=" overflow-hidden group-[imghov] relative m-6 pt-0 h-[650px]  max-w-[400px] w-fit    hover:scale-110  transition-all duration-300 delay-100 ease-in-out rounded-2xl flex justify-between flex-col items-center text-center text-white">
		<div class=" relative flex w-full	justify-between mb-2">
			<div class="relative pt-6 -z-[1]">
				<img src={SolsticeFullLogo} alt="logo" > 
			</div>
	
				<div class="  z-[1] h-0 w-0 mr-1 capitalize flex justify-center items-end border-[30px] border-t-[50px] border-x-[32px] bg-black bg-opacity-0 border-b-transparent {data.pass === 'flagship'? "border-[#9a0b0b8a]": data.pass==='standard'? "border-[#0e5f0e9b]": data.pass == "proshow"?"border-[#c59421cb]":"border-[#2564eb9c]"}" >
	...
	<style>
		.bgshadow{
			background-image: linear-gradient(to top, rgba(24,24,27,1),rgba(24,24,27,1),rgba(24,24,27,1),rgba(24,24,27,6),rgba(24,24,27,0.7),rgba(24,24,27,0));
		}
	</style>
